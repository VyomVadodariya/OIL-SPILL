...
Marine oil pollution poses severe environmental, ecological, and economic threats. Detecting slicks across thousands of square kilometers of open ocean and establishing which vessels were in the vicinity during discharge presents major operational difficulties:

- **Open-Ocean Scale & Cloud Cover:** Optical satellite sensors are frequently blocked by cloud cover, haze, and darkness.
- **Why Satellite SAR is Essential:** Synthetic Aperture Radar (SAR)—such as Sentinel-1 C-band radar—is an active microwave sensor that operates day and night and penetrates cloud cover. Oil films dampen ocean capillary waves, reducing radar backscatter and appearing as distinct dark patches ("dark spots") against the rougher sea surface.
- **Why AIS Alone is Insufficient:** Vessel Automatic Identification System (AIS) tracking reveals vessel positions, but spatial-temporal decoupling occurs immediately upon discharge. By the time a satellite acquisition takes place (often hours or days after discharge), ocean surface currents and surface winds have moved and deformed the slick by tens of kilometers. A vessel observed directly above a detected slick at observation time may not have been present during release, while the vessel present at release may already be far away.
- **Why Drift Modelling is Critical:** Hydrodynamic drift modeling in reverse (backtracking) simulates the historical trajectory of the slick under ocean currents and windage, estimating the spatial-temporal corridor where the discharge originally entered the water.
- **What OceanIntel Combines:** OceanIntel unifies Sentinel-1 SAR imagery ingestion, deep learning segmentation, morphological slick characterization, backward Lagrangian particle drift modeling, historical AIS trajectory correlation, and deterministic evidence fusion into a single decision-support platform.
- **What the Intelligence Output Represents:** OceanIntel produces ranked **Investigation Candidates** with transparent **Investigation Scores** and evidence completeness metrics. The system is designed strictly for decision support and does not establish legal guilt or regulatory culpability.

> [!IMPORTANT]
> **Decision-Support Boundary:** OceanIntel is a decision-support and investigation-intelligence tool for maritime authorities, environmental regulators, and coast guard personnel. It produces **Investigation Candidates** (e.g., *Highest-Priority Investigation Candidate*, *Candidate Vessel*), never "guilty vessels", "culprits", or "confirmed polluters". Observations such as AIS observation gaps indicate missing telemetry, not proof of intentional transponder deactivation. Final attribution requires ground-truth physical sampling, human inquiry, and independent evidence.
...
- ✅ **Implemented:** Fully implemented in the repository codebase.
- 🟡 **Prototype / Partial:** Functional skeleton, partial logic, or placeholder present.
- 🟠 **Simulated / Demo:** Implemented using mock, synthetic, or simulation fixtures.
- 🔵 **Planned:** Architectural design established; not yet connected or implemented.

---

## Core Capabilities

### 1. Satellite / SAR Intelligence
- Ingests single-band Sentinel-1 Synthetic Aperture Radar (SAR) Ground Range Detected (GRD) GeoTIFFs (primarily VV polarization).
- Converts raw linear amplitude/power values to decibels ($10 \log_{10}(x)$) and normalizes pixel values to a bounded $[0, 1]$ interval.
- Maintains spatial geospatial metadata (CRS, affine transform matrix, and pixel resolution) across the processing workflow.

### 2. AI Segmentation
- Implements a U-Net architecture with a ResNet-34 encoder pre-trained on ImageNet via `segmentation-models-pytorch`.
- Performs single-class binary segmentation, predicting candidate oil slick foreground against sea-surface background.
- Employs a sliding-window inference pipeline (256×256 window, 128-pixel stride, 50% overlap) with probability accumulation blending to suppress border artifacts across full-scene rasters.

### 3. Spill Characterization
- Applies morphological opening and closing kernels to remove isolated speckle false positives and fill interior slick voids.
- Extracts individual connected slick entities via `scipy.ndimage` and `skimage.measure.regionprops`.
- Computes morphological metrics including pixel area, physical surface area ($\text{km}^2$ for projected coordinate systems), perimeter ($\text{km}$), major/minor axis lengths, elongation ratio, and compactness index.
- Vectorizes detected raster masks into GeoJSON polygons using `rasterio.features.shapes`.

### 4. Drift Intelligence
- Simulates surface slick transport using a 2D kinematic particle integration model over the WGS84 ellipsoid.
- Incorporates combined environmental transport: $\vec{v}_{\text{total}} = \vec{v}_{\text{current}} + (\alpha_{\text{windage}} \cdot \vec{v}_{\text{wind}}) + \vec{v}_{\text{diffusion}}$.
- Supports forward spreading and backward trajectory backtracking (inverting velocity vectors) to project historical slick release corridors and origin time windows.
...
- Calculates spatial compatibility between vessel trajectories and backtrack corridors using configurable perpendicular distance decay (default 50 km) with corridor intersection boosts.
- Computes temporal compatibility based on proportional observation overlap with the estimated discharge window.
- Evaluates track quality, observation frequency, trajectory continuity, and corridor dwell time.

### 6. Evidence Fusion
- Deterministically fuses multi-modal indicators (spatial, temporal, drift, vessel motion, track quality) into a single scalar **Investigation Score**.
...
- Dynamically renders GeoJSON layers for detected oil slicks, backward drift trajectory envelopes, candidate vessel routes, historical points, and exclusion zones.
- Provides interactive measurement tools, layer toggles, and coordinate readouts.
...
|---|---|---|
| **SAR Preprocessing** | 🟡 Prototype / Partial | Linear-to-dB scaling and $[0, 1]$ normalization implemented; `apply_lee_filter` is a placeholder; radiometric calibration and terrain correction are not implemented. |
| **AI Segmentation** | ✅ Implemented | ResNet-34 U-Net binary segmentation (`classes=1`) implemented in PyTorch with Albumentations augmentation. |
| **Model Training** | ✅ Implemented | Training loop with dynamic positive-class weighting and scene-aware splitter implemented; 5 epochs trained on Gulf of Mexico dataset. Checkpoints are not committed to git. |
...
| **Drift Modelling** | 🟠 Simulated / Demo | 2D kinematic Euler particle tracking with windage and diffusion implemented; currently driven by synthetic environmental fixtures (`SyntheticConstantEnv`, `SyntheticVariableEnv`). Real CMEMS/ERA5 integration is planned. |
| **AIS Correlation** | 🟡 Prototype / Partial | `OfflineAISProvider` (CSV/in-memory) and scoring logic implemented; `AISStreamProvider` and `GFWProvider` are skeleton stubs. |
...
| **Backend API** | ✅ Implemented | FastAPI application with `GET /health` and `POST /api/v1/inference` for GeoTIFF raster processing. |
| **Map Engine** | ✅ Implemented | MapLibre GL JS with OpenFreeMap vector tile basemaps (Liberty/Bright/Dark); renders GeoJSON layers and vessel tracks. Google Maps is not active or configured. |
| **Dashboard UI** | ✅ Implemented | 13 functional workstations in React 19 + TypeScript; includes demo store and live SAR inference upload integration. |
| **Database / PostGIS** | 🔵 Planned | Relational database schema and PostGIS spatial storage are not implemented; data is handled in-memory and via filesystem artifacts (GeoTIFF, GeoJSON, CSV). |
...
| **Model Checkpoint** | `models/best_model.pth` (Not tracked in git; generated via `create_dummy_model.py` or `train.py`) |

> [!IMPORTANT]
> **Segmentation Classes — Current vs. Planned:**
> Current implementation uses **binary segmentation** (`classes=1`). The model detects anomalous oil slick backscatter against ocean background. Five-class segmentation (distinguishing oil slicks, look-alikes, vessels, land, and sea surface) was explored in early project design documents and dataset audits, but is **planned for future iterations** and is not currently implemented in the codebase.
...
#### Held-Out Test Scene Evaluation
...
- **Uncertainty Annotations:** The `SpillDetection` data schema tracks explicit uncertainty flags for model confidence, segmentation uncertainty, geolocation certainty, and attribution confidence.
...
- **Current State:** The drift module is a **stochastic/kinematic prototype** driven by synthetic test fixtures (`SyntheticConstantEnv`, `SyntheticVariableEnv`).
- **Planned Real Forcing:** Real-time and historical integration with Copernicus Marine Service (CMEMS) hydrodynamic ocean currents and ECMWF ERA5 atmospheric wind reanalysis is architecturally defined via `BaseEnvironmentalAdapter` but **not yet connected**.
- **Important:** OceanIntel does not claim operational oceanographic forecasting capability at this prototype stage.
...
## Geospatial Visualization / Map Engine
...
- **Google Maps Integration Status:** The active map visualization in this repository is powered by MapLibre GL and OpenFreeMap. Google Maps is **not active or configured** in the frontend source code. No API keys are embedded or exposed.
...
|---|---|---|---|---|
| `GET` | `/health` | Service health status check | None | `{"status": "ok"}` |
| `POST` | `/api/v1/inference` | Executes full-scene SAR segmentation and characterization on an uploaded GeoTIFF | `multipart/form-data`<br>Field: `file` (`.tif` or `.tiff`) | JSON object with detection status, object count, extracted geometries, and output mask path |
...
The repository contains approximately 67 automated tests spanning 12 test modules:
...
> [!NOTE]
> **Engineering Validation vs. Real-World Accuracy:** Passing tests confirm deterministic software correctness, mathematical transformation integrity, and pipeline data contracts. They do **not** prove real-world detection accuracy across complex marine environments.
